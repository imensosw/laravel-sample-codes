...
<div class="right_col" role="main" style="margin-left: 70px !important; padding: 0 !important;">
  <div class="container-fluid gutter">
    ...
    <div id="dialog" title="Add New">
        ...
        <div>
            <label for="txtName">Enter Name</label><br /><input id="txtName" placeholder="Enter name" class="form-control" type="text" />
        </div><br />
...
{
  $nodes .='{ id: "units'.$unit['id'].'", parentId: null , table: "units" , name: "'.$unit['unitName'].'", title: "'.$unit['unitName'].'" , item: "( Unit )"},';
}
foreach ($divisions as $division) 
{
 $nodes .='{ id:"divisions'.$division['id'].'", parentId:"units'.$division['unitId'].'" , table: "divisions" , name: "'.$division['divisionName'].'", title: "'.$division['divisionName'].'", item: "( Division )"},';
}
foreach ($departments as $department) 
{
 $nodes .='{ id:"departments'.$department['id'].'", parentId:"divisions'.$department['divisionId'].'" , table: "departments" , name: "'.$department['departmentName'].'", title: "'.$department['departmentName'].'", item: "( Department )"},';
}
foreach ($sub_departments as $sub_department) 
{
$nodes .='{ id:"sub_departments'.$sub_department['id'].'", parentId:"departments'.$sub_department['departmentId'].'" , table: "sub_departments" , name: "'.$sub_department['subDepartmentName'].'", title: "'.$sub_department['subDepartmentName'].'", item: "( Sub department )"},';
}
foreach ($job_roles as $job_role) 
{
$nodes .='{ id:"job_roles'.$job_role['id'].'", parentId:"sub_departments'.$job_role['subDepartmentId'].'" , table: "job_roles" , name: "'.$job_role['jobRoleName'].'", title: "'.$job_role['jobRoleName'].'", item: "( Role )"},';
}
$nodes .=']';
...
?>
    <script type="text/javascript">
       ...
       function insertNodeEvent(sender, args) 
       {
            ...
            if( args.node.pid.split('units')[1]  >  0 )
            {
                args.node.data["item"] = "( Division )";
            }
            else if( args.node.pid.split('divisions')[1]  >  0 )
            {
                args.node.data["item"] = "( Department )";
            }
            else if( args.node.pid.split('departments')[1]  >  0 )
            {
                args.node.data["item"] = "( Sub Department )";
            }
            else if( args.node.pid.split('sub_departments')[1]  >  0 )
            {
                args.node.data["item"] = "( Role )";
            } 
        }
                ...
                btns[i].addEventListener("click", function () {                  
                    ...
                    switch (action) 
                    {
                        case "add":
                            ...
                            if( node.data.table == "job_roles" || node.data.table == undefined)
                            {
                                break;
                            }
                        ...
                        case "delete":
                            var node_id = node.id ;
                            if( node.data.table == undefined)
                            {
                                orgChart.removeNode(node_id);
                                break;
                            }
                            ...
                            $.ajax({
                                type: "POST",
                                url: "{{ url('') }}/"+"nodeDelete",
                                data: { "_token": "{{ csrf_token() }}" , nid : node.id , ntable : node.data.table },
                                dataType: "json",
                    ...
                    var node1 = orgChart.nodes[document.getElementById("hdnId").value];
                    //alert(node1.pid);
                    var result = node1.pid.split('sub_departments');
                    ...
                    else if( node1.pid.split('sub_departments')[1]  >  0 )
                    {
        ...
        document.getElementById("btnSave").addEventListener("click", function () 
        {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('') }}/"+"nodesUpdate",
                data: { "_token": "{{ csrf_token() }}" , nid : node.id , pid : node.pid , ntable : node.data.table , ptable : node.parent.data , name : node.data.name , title : node.data.title , mail : node.data.mail , car : node.data.car , hasDriverLicense : node.data.hasDriverLicense , jobRoleGradeId :   document.getElementById("jobRoleGradeId").value},
                dataType: "json",
                success: function(data)
                {
                  if(data.status=="success")
                  {
                    ...
                    //     $id => "job_roles".$job_roles->id,
                    //     $parentId => "sub_departments".$id,
                    //     $table => "job_roles" ,
                    //     $name => $_POST['name'] ,
                    //     $title => $_POST['name'] ,
                    //     $item = > "( Role )"
                    // );
                    //alert(data.parentId);
                    orgChart.updateNode(id, pid,  { id:data.id , parentId:data.parentId , table:data.table ,  name:data.name ,title:data.title ,  item:data.item } );
                     showMessage( data.message, 'success',10000);
